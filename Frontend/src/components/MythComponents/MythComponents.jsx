...
const MythComponents = () => {
  ...
  const loadMyths = async () => {
    ...
    if (auxData.status === "Failed") {
      setErrorMessage(auxData.message);
      return;
    }
    ...
    <div>
      {!myths || myths.length === 0 ? (
        errorMessage !== "" ? (
          ...
          <h2>No hay una base de datos para cargar</h2>
        )
      ) : (
        <>
          <div className="total-myth-grid">
            {myths.map((m, idx) => {
              return (
                <div
                  key={idx}
                  className="myth-display"
                  onClick={() => pantheonClick(m?.pantheon)}
                >
                        ...
                        <div className="myth-symbols-display">
                          {m?.majorGod?.symbols.map((s, majorIdx) => {
                            return (
                              <div
                                key={majorIdx}
                                className="myth-symbols-display-individual"
                              >
                                <p>{s}</p>
                              </div>
                            );
                  ...
                  <div className="myth-manuscript-div">
                    <p>
                      ...
                      <span className="myth-manuscript-name">
                        {m?.manuscript?.name}
                      </span>
                    </p>
                  </div>
                  <div className="myth-representation">
                    <img
                      className="myth-representation-img"
